...
# function to convert duration to months
def dur_mon (totdur):
    if 'year' in totdur:
...
# A function to convert city names to locations(e.x., 'Redwood City' to 'San Fransisco Bay Area')
...
#Dictionary of locations and cities in that location
ctm = {}
...
try:
    for profno in range(1,len_users+1):
        ...
        driver.get('file:///home/sameer/linkedin-scraper/uber_profiles/'+ str(profno) + '.html')
        time.sleep(5)
        if 1:
            print userno,
            try:
...
wb.save('uber_analysis_raw.xlsx')

print '\nScraping completed ( ͡° ͜ʖ ͡°)'